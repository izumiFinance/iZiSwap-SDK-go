...
func SwapY2XDesireX(desireX *big.Int, highPt int, pool PoolInfo) (SwapResult, error) {
	if desireX.Cmp(big.NewInt(0)) <= 0 {
		return SwapResult{}, fmt.Errorf("XP")
	}
	...
	for currentPoint < highPt && !finished {
		if orderData.IsLimitOrder(currentPoint) {
			...
			feeAmount := calc.MulDivCeil(costY, big.NewInt(int64(fee)), big.NewInt(int64(1e6-fee)))
			if desireX.Cmp(acquireX) <= 0 {
				desireX = big.NewInt(0)
			} else {
		...
		if liquidity.Cmp(big.NewInt(0)) == 0 {
			...
			if desireX.Cmp(big.NewInt(0)) > 0 {
				...
				retState := swapmathdesire.Y2XRange(st, nextPoint, sqrtRate_96, new(big.Int).Set(desireX))

				finished = retState.Finished
				feeAmount := calc.MulDivCeil(retState.CostY, big.NewInt(int64(fee)), big.NewInt(int64(1e6-fee)))

				amountX.Add(amountX, retState.AcquireX)
				amountY.Add(amountY, new(big.Int).Add(retState.CostY, feeAmount))
				desireX.Sub(desireX, calc.MinBigInt(desireX, retState.AcquireX))
